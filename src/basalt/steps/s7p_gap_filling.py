...
def parse_sam_bwa(sam_file, fq, pair, n, mp_run, num_threads=4):
    """
    ...
    for line in open(sam_file,'r'):
        ...
        if slot is None:
            ...
            if slot[0] == 2:
                fq_seq=flist[9]+'\n+\n'+flist[10]+'\n'
                slot[2]='@'+rid+' 2\n'+fq_seq
            # count > 2 → leave as-is, will be rejected as multimapper at end
...
def mapping_extraction(lr_elongated_bin_folder, datasets_list, num_threads, pwd, mp_run):
    ...
    for item in datasets_list.keys():
        ...
        parse_sam_bwa(str(item)+'.sam', fq, pair, n, mp_run, num_threads=num_threads)
