...
"""
Step S6: Contig retrieval from PE-supported connections.

Unified entry for CheckM2 and CheckM backends. Call ``set_qc_backend(name)``
before invoking ``Contig_recruiter_main`` to select the QC tool.
"""
...
def _require_backend():
    if _BACKEND is None:
        raise RuntimeError(
            'S6_retrieve_contigs_from_PE_contigs: QC backend not set. '
...
def _bin_comparison_checkm2(original_bin_folder, new_bins_checkm, new_bin_folder, refinement_mode, num_threads):
    backend = _require_backend()
    pwd=os.getcwd()
    print('Comparing bins before and after refining process (CheckM2 logic)')
    ...
    if refinement_mode == 'deep':
        ...
        #     f_dr.write('6th bins quality check done'+'\n')
            ...
            if lp < 2:
                ...
                os.system(backend.build_cmd(num_threads, 'Deep_retrieved_bins_split', 'Deep_retrieved_bins_split_checkm', ext='fa'))
                # n = 0
            ...
            if lp < 3:
                refined_metrics = backend.parse_results(pwd+'/Deep_retrieved_bins_split_checkm')
                ...
                for refined_bin_id_raw, m in refined_metrics.items():
                    ...
                    if True:
                        ...
                        if refined_delta > ori_delta:
                            ...
                            new_bins_checkm[refined_bin_id]['Genome size']=int(m.get('Genome size', 0))
                            new_bins_checkm[refined_bin_id]['contig_size']=float(m.get('contig_size', 0.0))
                            bestbin[org_bin_id][refined_bin_id]=new_bins_checkm[refined_bin_id]
                            os.system('cp '+pwd+'/Deep_retrieved_bins_split/'+normalise_bin_filename(refined_bin_id)+' '+pwd+'/'+str(new_bin_folder))
                            try:
                                ...
                                for record in SeqIO.parse(normalise_bin_filename(refined_bin_id),'fasta'):
                                    if record.id not in bin_seq_rec[org_bin_id].keys():
                                        try:
            ...
            if lp < 4:        
                ...
                if xxxx>=1:
                    os.system(backend.build_cmd(num_threads, 'Deep_retrieved_bins', 'Deep_retrieved_bins_checkm', ext='fa'))
                    refined_metrics = backend.parse_results(pwd+'/Deep_retrieved_bins_checkm')
                    ...
                    for refined_bin_id_raw, m in refined_metrics.items():
                        ...
                        if refined_delta > org_delta:
                            ...
                            bestbin[org_bin_id][refined_bin_id]=new_bins_checkm[refined_bin_id]
                            try:
                                os.system('cp '+pwd+'/Deep_retrieved_bins/'+normalise_bin_filename(refined_bin_id)+' '+pwd+'/'+str(new_bin_folder))
                            except:
    ...
    for item in selected_bin.keys():
        if '_RT-D' in item:
            ...
            item_name=str(item)
        m = total_bin_checkm[item]
        cs = m.get('contig_size', m.get('N50', 0))
        f_bin_checkm.write(str(item_name)+'\t'+str(m.get('Genome size', 0))+'\t'+str(m.get('Completeness', 0))+'\t'+str(m.get('Contamination', 0))+'\t'+str(cs)+'\n')
    f_bin_checkm.close()
...
def coverage_filtration(bin, m):
    contig_filtration(bin, 'coverage_filtration', m, float(1.5), int(1), os.getcwd())

def TNF_filtration(bin, m):
    contig_filtration(bin, 'TNF_filtration', m, float(1.5), int(1), os.getcwd())
...
def kmer_cal(input_file, output_file):
    os.system('perl '+_script_path('calc.kmerfreq.pl')+' -i '+str(input_file)+' -o '+str(output_file))
...
def _parse_bin_in_bestbinset_checkm2(assemblies_list, binset, outlier_remover_folder, PE_connections_list, lr_connection_list, num_threads, last_step, coverage_matrix_list, refinement_mode):
    pwd=os.getcwd()
    assemblies={}
    ...
    if int(last_step) < 6:
        ...
        os.system(backend.build_cmd(num_threads, str(binset)+'_retrieved', str(binset)+'_retrieved_checkm', ext='fa'))
        f_cp=open('S6_checkpoint.txt','a')
    ...
    if int(last_step) < 7:
        ...
        bins_checkm = {}
        try:
            raw = backend.parse_results(pwd+'/'+str(binset)+'_retrieved_checkm')
            for binID, m in raw.items():
                ...
                if 'marker lineage' in m:
                    bins_checkm[bin_id]['marker lineage'] = m['marker lineage']
        except Exception as e:
            print('Could not parse retrieved-bins QC results:', e)
...
def binset_filtration(binset, pwd, cpn_cutoff, ctn_cutoff):
    ...
    for binID, m in filtrated_checkm.items():
        f.write(binID+'\t'+str(m.get('Genome size', 0))+'\t'+str(m.get('Completeness', 0))+'\t'+str(m.get('Contamination', 0))+'\t'+str(m.get('contig_size', 0))+'\n')
    f.close()
    os.chdir(pwd)
...
def Contig_recruiter_main(binset, outlier_remover_folder, num_threads, parameter, cpn_cutoff, ctn_cutoff, assemblies_list, PE_connections_list, coverage_matrix_list, refinement_mode, pwd, lr_connection_list=None):
    if lr_connection_list is None:
        lr_connection_list = []
    ...
    parse_bin_in_bestbinset(assemblies_list, binset+'_filtrated', outlier_remover_folder, PE_connections_list, num_threads, last_step, coverage_matrix_list, refinement_mode, lr_connection_list=lr_connection_list)
...
def bin_comparison(original_bin_folder, new_bins_checkm, new_bin_folder, refinement_mode, num_threads):
    """Public dispatcher: routes to backend-specific bin_comparison helper."""
    backend = _require_backend()
    if backend.name == 'checkm':
        return _bin_comparison_checkm(original_bin_folder, new_bins_checkm, new_bin_folder, refinement_mode, num_threads)
    return _bin_comparison_checkm2(original_bin_folder, new_bins_checkm, new_bin_folder, refinement_mode, num_threads)


def parse_bin_in_bestbinset(assemblies_list, binset, outlier_remover_folder, PE_connections_list, num_threads, last_step, coverage_matrix_list, refinement_mode, lr_connection_list=None):
    """Public dispatcher: routes to backend-specific parse_bin helper.
    Note: ``lr_connection_list`` is only used by the CheckM2 path."""
    backend = _require_backend()
    if backend.name == 'checkm':
        return _parse_bin_in_bestbinset_checkm(assemblies_list, binset, outlier_remover_folder, PE_connections_list, num_threads, last_step, coverage_matrix_list, refinement_mode)
    if lr_connection_list is None:
        lr_connection_list = []
    return _parse_bin_in_bestbinset_checkm2(assemblies_list, binset, outlier_remover_folder, PE_connections_list, lr_connection_list, num_threads, last_step, coverage_matrix_list, refinement_mode)
...
if __name__ == '__main__':
    ...
    num_threads=60
    refinement_mode='deep'
    cpn_cutoff=35
    ctn_cutoff=20
    pwd=os.getcwd()
    parameter='last'
    Contig_recruiter_main(best_binset_from_multi_assemblies, outlier_remover_folder, num_threads, parameter, cpn_cutoff, ctn_cutoff, assemblies_list, PE_connections_list, coverage_matrix_list, refinement_mode, pwd, lr_connection_list=lr_connection_list)
def _bin_comparison_checkm(original_bin_folder, new_bins_checkm, new_bin_folder, refinement_mode, num_threads):
    pwd=os.getcwd()
                ...
                if xxxx == 0:
                    if split_folder != 1:
                        ...
                        for f_name in floder_name.keys():
                            checkm_tax_wf(str(marker_lineage_level_name), str(marker_lineage_taxon), int(num_threads), str(f_name))
                        ...
                        if marker_lineage_taxon != 'algicola':
                            checkm_tax_wf(str(marker_lineage_level_name), str(marker_lineage_taxon), int(num_threads), str(org_bin_id)+'_deep_retrieval')
                            if os.path.exists(pwd+'/'+str(org_bin_id)+'_deep_retrieval_checkm/storage/bin_stats_ext.tsv'):
    ...
    os.chdir(pwd)   
    print('Contig retrieve done!')
def _parse_bin_in_bestbinset_checkm(assemblies_list, binset, outlier_remover_folder, PE_connections_list, num_threads, last_step, coverage_matrix_list, refinement_mode):
    pwd=os.getcwd()
    assemblies={}
    for item in assemblies_list:
        assemblies[item]=[]
    ...
    if int(last_step) < 7:
        ...
        os.chdir(pwd)
        _bin_comparison_checkm(str(binset), bins_checkm, str(binset)+'_retrieved', refinement_mode, num_threads)
