...
"""
Step S7: Within-group contig retrieval.

Unified entry for CheckM2 and CheckM backends. Call ``set_qc_backend(name)``
before invoking ``Contig_retrieve_within_group`` to select the QC tool.
"""
...
def _require_backend():
    if _BACKEND is None:
        raise RuntimeError(
            'S7_Contigs_retrieve_within_group: QC backend not set. '
...
def checkm(bin_folder, num_threads):
    """Run the configured QC backend on ``bin_folder`` and parse results."""
    backend = _require_backend()
    pwd=os.getcwd()
    os.system('rm -rf '+str(bin_folder)+'_checkm')
    os.system(backend.build_cmd(num_threads, str(bin_folder), str(bin_folder)+'_checkm', ext='fa'))
...
def _bin_comparison_checkm2(original_bin_folder, new_bins_checkm, new_bin_folder,
                   num_threads, total_contigs, level_num):
    """
    Compare original bins and retrieved bins to select improved versions.

    The function combines CheckM2 quality metrics with contig-level
    ...
    num_threads : int
        Number of threads (kept for API symmetry; CheckM2 call already done).
    total_contigs : dict
    ...
    for item in bin_comparison_list.keys():
        ...
        for i in range(0, len(bin_comparison_list[item])):
            if i == 0:
                ...
                re_taxon=str(new_bins_checkm[refined_bin_id].get('contig_size', new_bins_checkm[refined_bin_id].get('N50', 0)))
                ...
                for bin_id2 in bestbin[item].keys():
                    ...
                    ori_taxon=str(bestbin[item][bin_id2].get('contig_size', bestbin[item][bin_id2].get('N50', 0)))
    ...
    for org_bin_id in further_refined_bin_contig.keys():
        ...
        if org_bin_id in deep_metrics:
            ...
            org_bin_ctn=float(deep_metrics[org_bin_id].get('Contamination', 0.0))
            ori_delta=org_bin_cpn-5*org_bin_ctn
            ori_1delta=org_bin_cpn-org_bin_ctn
            for refined_bin_id, m in deep_metrics.items():
        ...
        for refined_bin_id_raw, m in refined_metrics.items():
            ...
            if refined_delta > org_delta:
                ...
                new_bins_checkm[refined_bin_id]={
                    'Completeness': refined_bin_cpn,
                    'Contamination': refined_bin_ctn,
                    'Genome size': int(m.get('Genome size', 0)),
                    'contig_size': float(m.get('contig_size', 0.0)),
                }
...
def coverage_filtration(bin, m, level_num, cutoff):
    contig_filtration(bin, 'coverage_filtration', m, float(cutoff), int(level_num), os.getcwd())

def TNF_filtration(bin, m, level_num, cutoff):
    contig_filtration(bin, 'TNF_filtration', m, float(cutoff), int(level_num), os.getcwd())
...
def Contig_retrieve_within_group(assemblies_list, binset, outlier_remover_folder,
    ...
    This orchestrates the S7 step for the CheckM2 branch by:
    ...
    5. Updating bins and CheckM2 quality reports across iterations.
...
def Contig_retrieve_within_group_main(binset, outlier_remover_folder,
    ...
    """
    Entry point for S7 within-group contig retrieval (CheckM2 branch).
...
# --- CheckM-specific helper (used only when backend=="checkm") ---

def _bin_comparison_checkm(original_bin_folder, new_bins_checkm, new_bin_folder,
                   num_threads, total_contigs, level_num):
    """
    Compare original bins and retrieved bins to select improved versions.
    ...
    for org_bin_id in further_refined_bin_contig.keys():
        ...
        if os.path.exists(pwd+'/'+str(org_bin_id)+'_deep_retrieval_checkm/storage/bin_stats_ext.tsv'):
            ...
            xxxx=0
            os.system('rm -rf '+str(org_bin_id)+'_deep_retrieval_checkm')
        print(str(bin_checkm[str(org_bin_id)]['marker lineage']))
        print(str(marker_lineage_level_name))
    ...
    replace_bin = {}
    if xxxx>=1:
        os.system('checkm lineage_wf -t '+str(num_threads)+' -x fa Deep_retrieved_bins_'+str(level_num)+' Deep_retrieved_bins_'+str(level_num)+'_checkm')
...
def bin_comparison(original_bin_folder, new_bins_checkm, new_bin_folder, num_threads, total_contigs, level_num):
    ...
    if backend.name == "checkm":
        return _bin_comparison_checkm(original_bin_folder, new_bins_checkm, new_bin_folder, num_threads, total_contigs, level_num)
    return _bin_comparison_checkm2(original_bin_folder, new_bins_checkm, new_bin_folder, num_threads, total_contigs, level_num)
