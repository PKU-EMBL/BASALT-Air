...
"""
Step S4: Multi-assembly comparison and dereplication.

Unified entry for CheckM2 and CheckM backends. Call ``set_qc_backend(name)``
before invoking ``multiple_assembly_comparator_main`` to select the QC tool.
"""
...
def _require_backend():
    if _BACKEND is None:
        raise RuntimeError(
            'S4_Multiple_Assembly_Comparator: QC backend not set. '
            "Call set_qc_backend('checkm2' or 'checkm') first."
        )
    return _BACKEND


# Coverage matrix files are reparsed by ``genome_contigs_recorder``,
# ``new_selected_bins_generator`` and ``binset_comparator`` on every
# iteration of the main de-rep loop. Cache the parsed view keyed by file
# path so each file is read from disk and tokenised once per process.
_COVERAGE_CACHE = {}


def _parse_coverage(path):
    """Return a cached parsed view of a coverage matrix file.

    The structure is:
      ``num_drange``   - int, count of ``drange`` tokens in the header line
      ``header_line``  - the raw header line (newline-stripped)
      ``rows``         - list of dicts ``{'id', 'raw', 'fields'}`` in file order
      ``by_id``        - dict mapping contig_id -> the same row dict

    Each row's ``fields`` is the pre-split tab-separated tokens, so callers
    that previously did ``str(line).strip().split('\\t')`` can now reuse a
    single split per line across all four hot consumers.
    """
    ...
    with open(path, 'r') as fh:
        for n, line in enumerate(fh):
            if n == 0:
                ...
                num_drange = line.count('drange')
                continue
            stripped = line.strip()
            if not stripped:
                continue
            fields = stripped.split('\t')
            row = {'id': fields[0], 'raw': stripped, 'fields': fields}
            rows.append(row)
            by_id[fields[0]] = row
    parsed = {
        'num_drange': num_drange,
        'header_line': header_line,
        'rows': rows,
        'by_id': by_id,
    }
    _COVERAGE_CACHE[path] = parsed
    return parsed
...
def core_contigs_filtration(bin_contig_cov, bin_contig, contig_cov,
    ...
    num_coverage=0
    for bin_cov in bin_contig_cov.values():
        for cov_values in bin_cov.values():
            num_coverage=max(num_coverage, len(cov_values))
    if num_coverage == 0:
        print('No bins with coverage detected in '+str(folder_binset)+'. Skipping core contig filtration.')
        return {}, {}
...
def TNFs_refiner(binset, assembly, coverage_refined_folder, threshold,
    ...
    os.system('perl '+_script_path('calc.kmerfreq.pl')+' -i '+str(assembly)+' -o '+str(assembly)+'.kmer.txt')
...
def genome_contigs_recorder(binset, binset_record, binset_genome_size,
    ...
    parsed=_parse_coverage(coverage_matrix)
    num=parsed['num_drange']
...
def coverage_GC_comparator(binset_coverage_avg1, binset_coverage_avg2,
...
def seq_comparator(blast_output, binset1, binset2, seq_len1, seq_len2,
...
def checkm_connections(binset):
    """Read QC stats for a given binset using the configured backend."""
    backend = _require_backend()
    print('Reading '+backend.name+' output of '+binset)
    pwd=os.getcwd()
    ...
    print('Done of reading '+backend.name+' output of '+binset)
    print('-----------------------------------------')
    return binset_checkm_connection

def _bin_comparator_checkm2(bin_seq_coverage_filtrated,
                   binset_checkm_connection_1,
                   binset_checkm_connection_2,
                   num):
    """
    Compare bins from two binsets using coverage and quality metrics
    (CheckM2 logic — Completeness − 5*Contamination tie-breaker, no marker lineage).
    """
...
def new_selected_bins_generator(selected_bins, eliminated_bins,
    ...
    parsed1=_parse_coverage(coverage_matrix1)
    coverage_title=parsed1['header_line']
    for row in parsed1['rows']:
        coverage_maxtrix[row['id']]=row['raw']

    parsed2=_parse_coverage(coverage_matrix2)
...
def final_iteration_mapping(contigs, datasets, num_threads, pwd):
    ...
    for line in open(str(contigs)+'.depth.txt','r'):
        n+=1
        if n == 1:
            coverage_num=sum(1 for f in str(line).rstrip("\r\n").split("\t") if f.endswith("-var"))
        else:
            bin_id=str(line).strip().split('\t')[0].split('|')[0].strip()
...
def parse_checkm(file_or_folder):
    """Parse QC results into a bin_checkm dict.

    Accepts either a flat-TSV file path (legacy CheckM2 quality_report.tsv)
    OR a folder; in either case, the configured backend is used to extract
    metrics. Each bin is keyed by both ``<id>.fa`` and a stripped form for
    legacy compatibility.
    """
    backend = _require_backend()
    if os.path.isfile(file_or_folder):
        # Legacy: caller passes a TSV file path. Use parent dir for backend walk.
        folder = os.path.dirname(file_or_folder) or '.'
    else:
    ...
    for binID, m in raw.items():
        ...
        bin_checkm[bin_id_f] = entry
        bin_checkm[bin_id_f3] = dict(entry)
    return bin_checkm

def _initial_drep_final_comparator_checkm2(final_iteration_folder,
                                  Coverage_maxtrix_list, datasets,
                                  num_threads, pwd, step):
    """
    Compare bins during initial dereplication (CheckM2 logic).
    """
...
def _final_binset_comparator_checkm2(final_iteration_folder,
                            Coverage_maxtrix_list, datasets,
                            num_threads, pwd, step):
    """
    Final binset comparison after reassembly and OLC steps (CheckM2 logic).
    """
    ...
    if y == 0:
        backend = _require_backend()
        os.chdir(pwd)
        os.system('rm -rf '+str(final_iteration_folder)+'_checkm')
        os.system(backend.build_cmd(num_threads, str(final_iteration_folder), str(final_iteration_folder)+'_checkm', ext='fa'))
        bin_checkm.update(parse_checkm(pwd+'/'+str(final_iteration_folder)+'_checkm'))
...
def record_bin_coverage(best_binset_from_multi_assemblies, coverage_file):
    ...
    bin_checkm_temp=set()
    results_filename=_require_backend().results_filename
            ...
            if _require_backend().is_results_file(file):
                for line in open(file, 'r'):
                    ...
                    if not bins or bins in ('Bin_ID', 'Name'):
                        continue
                    bin_checkm_temp.add(bins)
                    bin_checkm_temp.add(bins+'.fa')
                    bin_checkm_temp.add(bins+'.fna')
                    bin_checkm_temp.add(bins+'.fasta')
    if bin_checkm_temp:
        print('Loaded '+str(len(bin_checkm_temp))+' allowed bin names from '+str(results_filename))
    else:
        print('No '+str(results_filename)+' found for '+str(best_binset_from_multi_assemblies)+'. Keeping all FASTA bins.')
    os.chdir(pwd)
    ####
    ...
    for root, dirs, files in os.walk(pwd+'/'+str(best_binset_from_multi_assemblies)):
        ...
        for file in files:
            ...
            if 'fa' in hz or 'fna' in hz:
                if bin_checkm_temp and str(file) not in bin_checkm_temp:
                    continue
                m+=1
                bin_contigs[file]={}
                bin_contigs_mock[file]={}
    ...
    contig_cov={}
    parsed=_parse_coverage(coverage_file)
    num=0
    for row in parsed['rows']:
        ls=row['fields']
        num=int((len(ls)-4)/3)
        ...
        contig_cov[ids]={}
        for i in range(1,num+1):
            contig_cov[ids][i]=float(ls[3*i+1])
...
def _binset_comparator_checkm2(binset1, binset2, coverage1, coverage2,
    ...
    contigs_coverage={}
    parsed1=_parse_coverage(coverage1)
    num=parsed1['num_drange']
    for row in parsed1['rows']:
        ids=row['id']
        fields=row['fields']
        contigs_coverage[str(ids)]={}
    ...
    parsed2=_parse_coverage(coverage2)
    num=parsed2['num_drange']
    for row in parsed2['rows']:
        ids=row['id']
        fields=row['fields']
        contigs_coverage[str(ids)]={}
...
def multiple_assembly_comparator_main(Contig_list_o, BestBinSet_list_o,
    ...
    if step == 'initial_drep':
        ...
        valid_BestBinSet_list, valid_Contig_list, valid_Coverage_list = [], [], []
        for i in range(0, len(BestBinSet_list)):
            ...
            if binset_folder2 not in processed:
                ...
                bin_contig=A[1]
                contig_cov=A[2]
                if not bin_contig_cov:
                    warning='No bins with coverage in '+str(binset_folder)+'. Skipping this binset during initial de-replication.'
                    print(warning)
                    try:
                        flog=open('Basalt_log.txt','a')
                    except:
                ...
                if not core_contigs:
                    warning='No core contigs could be generated for '+str(binset_folder)+'. Skipping this binset during initial de-replication.'
                    print(warning)
                    try:
                        flog=open('Basalt_log.txt','a')
                    except:
                ...
                flog.close()
            valid_BestBinSet_list.append(binset_folder)
            valid_Contig_list.append(contig_file)
            valid_Coverage_list.append(coverage_file)
        fx.close()

        BestBinSet_list=valid_BestBinSet_list
        Contig_list=valid_Contig_list
        Coverage_list=valid_Coverage_list
        contig_num=len(Contig_list)
        if contig_num == 0:
            print('Error! There is no qualitied bin with coverage after autobinning process. Please check your data')
            try:
                flog=open('Basalt_log.txt','a')
            except:
                flog=open('Basalt_log.txt','w')
            flog.write('Error! There is no qualitied bin with coverage after autobinning process. Please check your data'+'\n')
            flog.close()
            return

        if contig_num != 1:
            for i in range(1, contig_num):
                potential_simialr_bin=binset_comparator(BestBinSet_list[0], BestBinSet_list[-1], Coverage_list[0], Coverage_list[-1], binset_coverage_total, pwd, num_threads)
                ...
                selected_bins, eliminated_bins=bin_comparator(potential_simialr_bin, binset_checkm_connection_1, binset_checkm_connection_2, i)
        ...
        initial_drep_final_comparator(new_bin_folder, Coverage_list, datasets, num_threads, pwd, step)
...
if __name__ == '__main__': 
    ...
    num_threads=30
    multiple_assembly_comparator_main(Contig_list, BestBinSet_list, Coverage_list, datasets, step, num_threads)


# --- CheckM-specific helpers (used only when backend.name=="checkm") ---
# These four functions preserve GTDB marker_lineage tie-breaker logic that
# CheckM2 does not produce. Internal QC parsing here uses CheckM-format
# bin_stats_ext.tsv directly (correct behavior under CheckM backend).

def _bin_comparator_checkm(bin_seq_coverage_filtrated,
                   binset_checkm_connection_1,
                   binset_checkm_connection_2,
                   num):
    """
    ...
    for item in bin_seq_coverage_filtrated.keys():
        ...
        bin1_size=binset_checkm_connection_1[ID1]['Genome size']
        bin2_size=binset_checkm_connection_2[ID2]['Genome size']
        bin1_mean_len=binset_checkm_connection_1[ID1]['Mean scaffold length']
        bin2_mean_len=binset_checkm_connection_2[ID2]['Mean scaffold length']       
...
def _initial_drep_final_comparator_checkm(output_folder_name, final_iteration_folder,
                                  Coverage_maxtrix_list, datasets,
                                  num_threads, pwd, step):
    """
    Compare bins during initial dereplication (after S4) for CheckM branch.
    """
    # output_folder_name='BestBinset'
    try:
        os.system('mkdir '+output_folder_name)
    except:
        os.system('mv -r '+output_folder_name+' BestBinset_old')
        os.system('mkdir '+output_folder_name)
        ...
        for file in files:
            if 'bin_stats_ext.tsv' in file:
                ...
                for line in open(file, 'r'):
                    ...
                    bin_checkm_o[bin_id]=str(line).strip()
                    bin_checkm[bin_id_f]={}
                    bin_checkm[bin_id_f]['Connections']=int(str(line).strip().split('Connections\': ')[1].split('}')[0].split(',')[0].strip())
                    bin_checkm[bin_id_f]['marker lineage']=str(line).strip().split('marker lineage\': \'')[1].split('}')[0].split('\',')[0].strip()
                    bin_checkm[bin_id_f]['Completeness']=float(str(line).strip().split('Completeness\': ')[1].split('}')[0].split(',')[0].strip())
                    bin_checkm[bin_id_f]['Genome size']=float(str(line).strip().split('Genome size\': ')[1].split('}')[0].split(',')[0].strip())
                    bin_checkm[bin_id_f]['Mean scaffold length']=float(str(line).strip().split('Mean scaffold length\': ')[1].split('}')[0].split(',')[0].strip())
                    bin_checkm[bin_id_f]['Contamination']=float(str(line).strip().split('Contamination\': ')[1].split('}')[0].split(',')[0].strip())
                    bin_checkm[bin_id_f2]={}
                    bin_checkm[bin_id_f2]['Connections']=int(str(line).strip().split('Connections\': ')[1].split('}')[0].split(',')[0].strip())
                    bin_checkm[bin_id_f2]['marker lineage']=str(line).strip().split('marker lineage\': \'')[1].split('}')[0].split('\',')[0].strip()
                    bin_checkm[bin_id_f2]['Completeness']=float(str(line).strip().split('Completeness\': ')[1].split('}')[0].split(',')[0].strip())
                    bin_checkm[bin_id_f2]['Genome size']=float(str(line).strip().split('Genome size\': ')[1].split('}')[0].split(',')[0].strip())
                    bin_checkm[bin_id_f2]['Mean scaffold length']=float(str(line).strip().split('Mean scaffold length\': ')[1].split('}')[0].split(',')[0].strip())
                    bin_checkm[bin_id_f2]['Contamination']=float(str(line).strip().split('Contamination\': ')[1].split('}')[0].split(',')[0].strip())
    ...
    for item in filtrated_bin.keys():
        ...
        if bin_score_1 == bin_score_2:
            ...
            gz1=bin_checkm[ID1]['Genome size']
            gz2=bin_checkm[ID2]['Genome size']
            msl1=bin_checkm[ID1]['Mean scaffold length']
            msl2=bin_checkm[ID2]['Mean scaffold length']
...
def _final_binset_comparator_checkm(final_iteration_folder,
                            Coverage_maxtrix_list, datasets,
                            num_threads, pwd, step):
    """
    Final binset comparison after reassembly and OLC steps (CheckM branch).
    """
    output_folder_name=final_iteration_folder
    ...
    if y == 0:
        ...
        os.system('mv '+str(final_iteration_folder)+'_checkm/storage/bin_stats_ext.tsv '+pwd+'/'+final_iteration_folder)
        os.chdir(pwd+'/'+final_iteration_folder)
        bin_checkm.update(parse_checkm('bin_stats_ext.tsv'))
        ...
        for file in files:
            if 'bin_stats_ext.tsv' in file:
                ...
                for line in open(file, 'r'):
                    ...
                    bin_checkm[bin_id_f]['Completeness']=float(str(line).strip().split('Completeness\': ')[1].split('}')[0].split(',')[0].strip())
                    try:
                        bin_checkm[bin_id_f]['Genome size']=float(str(line).strip().split('Genome size\': ')[1].split('}')[0].split(',')[0].strip().replace('\'','').replace('\"',''))
                    except:
                        bin_checkm[bin_id_f]['Genome size']=float(str(line).strip().split('Genome size\': ')[1].split('}')[0].split(',')[0].strip())
                    bin_checkm[bin_id_f]['Contamination']=float(str(line).strip().split('Contamination\': ')[1].split('}')[0].split(',')[0].strip())
                    bin_checkm[bin_id_f2]={}
                    ...
                    bin_checkm[bin_id_f2]['Completeness']=float(str(line).strip().split('Completeness\': ')[1].split('}')[0].split(',')[0].strip())
                    try:
                        bin_checkm[bin_id_f2]['Genome size']=float(str(line).strip().split('Genome size\': ')[1].split('}')[0].split(',')[0].strip().replace('\'','').replace('\"',''))
                    except:
                        bin_checkm[bin_id_f2]['Genome size']=float(str(line).strip().split('Genome size\': ')[1].split('}')[0].split(',')[0].strip())
                    bin_checkm[bin_id_f2]['Contamination']=float(str(line).strip().split('Contamination\': ')[1].split('}')[0].split(',')[0].strip())
                    try:
                        bin_checkm[bin_id_f]['Mean scaffold length']=bin_status[bin_id_f]
                    except:
                        bin_checkm[bin_id_f2]['Mean scaffold length']=bin_status[bin_id_f2]
    os.chdir(pwd)
    ...
    for item in bin_checkm_o.keys():
        if item not in eliminated_bins_checkm.keys():
            if item not in replace2.keys():
                f.write(str(bin_checkm_o[item])+'\n')
                Completeness=str(bin_checkm_o[item]).split('Completeness\': ')[1].split(',')[0]
                Contamination=str(bin_checkm_o[item]).split('Contamination\': ')[1].split('}')[0].split(',')[0]
                delta_value=float(Completeness)-5*float(Contamination)
                if delta_value >= 70:
                    f3.write(str(bin_checkm_o[item])+'\n')
                elif delta_value >= 50:
...
def _binset_comparator_checkm(binset1, binset2, coverage1, coverage2,
                      binset_coverage_total, pwd, num_threads):
    """
    Compare two binsets and update selected bins and coverage summaries (CheckM).
    """
    bins_binset1, bins_binset2 = {}, {}
    for root, dirs, files in os.walk(pwd+'/'+binset1):
        os.chdir(pwd+'/'+binset1)
    ...
    blast_output=open('Filtrated_binset2_vs_binset1.txt','w')
    blast_output2=open('Filtrated_binset2_vs_binset1_test.txt','w')
    blast_output3=open('Filtrated_binset2_vs_binset1_test2.txt','w')
    bin1_cov_ave, bin2_cov_ave, bins_blast_file = {}, {}, {}
    # contig_num = 0
    for line in open('binset2_vs_binset1.txt','r'):
        simi=str(line).strip().split('\t')[2]
        if float(simi) >= 99:
            blast_output3.write(str(line))
            length=eval(str(line).strip().split('\t')[3])
            contig2=str(line).strip().split('\t')[0]
            if '||' not in contig2:
                org_contig2_id=str(contig2).split('|')[1].strip()
            ...
            if int(length) >= 1000 and iden_t >= 1:
                ...
                blast_output2.write(str(line).strip()+'\t'+str(contig2_length)+'\t'+str(contig1_length)+'\t'+str(iden2)+'\t'+str(iden1)+'\n')
    blast_output.close()
    blast_output2.close()
    blast_output3.close()
...
def bin_comparator(bin_seq_coverage_filtrated, binset_checkm_connection_1,
                   binset_checkm_connection_2, num):
    """Public dispatcher: routes to backend-specific bin_comparator."""
    backend = _require_backend()
    if backend.name == 'checkm':
        return _bin_comparator_checkm(bin_seq_coverage_filtrated, binset_checkm_connection_1, binset_checkm_connection_2, num)
    return _bin_comparator_checkm2(bin_seq_coverage_filtrated, binset_checkm_connection_1, binset_checkm_connection_2, num)


def initial_drep_final_comparator(final_iteration_folder, Coverage_maxtrix_list,
                                  datasets, num_threads, pwd, step,
                                  output_folder_name='BestBinset'):
    """Public dispatcher: routes to backend-specific initial_drep_final_comparator.
    ``output_folder_name`` is CheckM-specific; CheckM2 hardcodes it to 'BestBinset'."""
    backend = _require_backend()
    if backend.name == 'checkm':
        return _initial_drep_final_comparator_checkm(output_folder_name, final_iteration_folder, Coverage_maxtrix_list, datasets, num_threads, pwd, step)
    return _initial_drep_final_comparator_checkm2(final_iteration_folder, Coverage_maxtrix_list, datasets, num_threads, pwd, step)


def final_binset_comparator(final_iteration_folder, Coverage_maxtrix_list,
                            datasets, num_threads, pwd, step):
    """Public dispatcher: routes to backend-specific final_binset_comparator."""
    backend = _require_backend()
    if backend.name == 'checkm':
        return _final_binset_comparator_checkm(final_iteration_folder, Coverage_maxtrix_list, datasets, num_threads, pwd, step)
    return _final_binset_comparator_checkm2(final_iteration_folder, Coverage_maxtrix_list, datasets, num_threads, pwd, step)


def binset_comparator(binset1, binset2, coverage1, coverage2,
                      binset_coverage_total, pwd, num_threads):
    """Public dispatcher: routes to backend-specific binset_comparator."""
    backend = _require_backend()
    if backend.name == 'checkm':
        return _binset_comparator_checkm(binset1, binset2, coverage1, coverage2, binset_coverage_total, pwd, num_threads)
    return _binset_comparator_checkm2(binset1, binset2, coverage1, coverage2, binset_coverage_total, pwd, num_threads)
