...
"""
Step S10: OLC-based refinement after reassembly.

Unified entry for CheckM2 and CheckM backends. Call ``set_qc_backend(name)``
before invoking ``reassembly_OLC_main`` to select the QC tool.
"""
...
def elongate_contig_selector(eliminated_bin, threshold, pwd,
    ...
    in the post-reassembly stage (CheckM2 branch).
    ...
    try:
        ...
        # os.system('perl '+_script_path('calc.kmerfreq.pl')+' -i '+str(eliminated_bin)+' -o '+str(eliminated_bin)+'.kmer.txt')
        # os.system('perl '+_script_path('calc.kmerfreq.pl')+' -i '+pwd+'/'+eliminated_bin_containing_folder+'/'+eliminated_bin+' -o '+str(eliminated_bin)+'.kmer.txt')
        bin_TNFs_outlier={}
...
def outliner_remover(bin_id, contigs_ids, threshold, item_data, explained_variance_ratio, pwd):
    ...
    in the post-reassembly OLC step (CheckM2 branch).
...
def parse_checkm(test_bin_folder_checkm_containning_folder, pwd):
    """Parse QC results in ``test_bin_folder_checkm_containning_folder`` using the configured backend."""
...
def OLC_elongation_main(target_bin, eliminated_bin, target_bin_checkm,
    ...
    after reassembly (CheckM2 branch).
...
def reassembly_paired_bins(target_bin_folder, reassembly_binset_folder,
                           orig_binset):
    """
    Group reassembled bins that originate from the same original bin
    in the post-reassembly OLC stage (CheckM2 branch).
...
def mul_threads(item, bestbinset_sim_bin, bestbinset_checkm, step, pwd,
    ...
    # keep the file as-is, and mark them processed.
...
def cleanup(max_num):
    """
    Remove temporary OLC files generated in the post-reassembly stage.
...
def reassembly_OLC_main(target_bin_folder, step, bin_comparison_folder,
                        aligned_len_cutoff, similarity_cutoff,
                        coverage_extension, num_threads, ram, orig_binset):
    """
    High-level driver for the S10 post-reassembly OLC step (CheckM2 branch).
    ...
    bestbinset_checkm=parse_checkm(target_bin_folder, pwd)
    ...
    # Copy backend's results file (whatever its filename) into the OLC folder for record.
    src_results = backend.results_path(pwd+'/'+target_bin_folder+'_OLC_checkm')
    if os.path.exists(src_results):
        os.system('cp '+src_results+' '+pwd+'/'+target_bin_folder+'_OLC/OLC_'+backend.results_filename+'_o')

    os.chdir(pwd+'/'+target_bin_folder+'_OLC')
    fxyz=open('OLC_quality_report.tsv','w')
    fxyz.write('Bin_ID'+'\t'+'Genome_size'+'\t'+'Completeness'+'\t'+'Contamination'+'\t'+backend.contig_size_key+'\n')
    for bin_id in best_bin_checkm.keys():
        m = best_bin_checkm[bin_id]
        cs = m.get('contig_size', m.get('N50', 0))
        fxyz.write(str(bin_id)+'\t'+str(m.get('Genome size', 0))+'\t'+str(m.get('Completeness', 0))+'\t'+str(m.get('Contamination', 0))+'\t'+str(cs)+'\n')
    fxyz.close()
...
if __name__ == '__main__': 
    ...
    target_bin_folder='1_Opera_unpolished_cat_contigs.fasta_BestBinsSet_outlier_refined_filtrated_retrieved_MAGs_polished_re-assembly_bs2'
    ...
    bin_comparison_or_reassembly_binset_folder='1_Opera_unpolished_cat_contigs.fasta_BestBinsSet_outlier_refined_filtrated_retrieved_MAGs_polished_re-assembly_binset_bs2'
    orig_binset='1_Opera_unpolished_cat_contigs.fasta_BestBinsSet_outlier_refined_filtrated_retrieved_MAGs_polished_re-assembly_bs2_mod'
