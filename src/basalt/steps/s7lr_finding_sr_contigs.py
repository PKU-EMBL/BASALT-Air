...
"""
Step S7lr: Long-read-based contig retrieval and polishing.

Unified entry for CheckM2 and CheckM backends. Call ``set_qc_backend(name)``
before invoking ``finding_sr_contigs_basing_lr_main`` to select the QC tool.
"""
...
def _require_backend():
    if _BACKEND is None:
        raise RuntimeError(
            'S7lr_finding_sr_contigs: QC backend not set. '
...
def mod_bin(binset_folder, pwd):
    ...
    mod_tsv = 'Bin_name_mod_'+backend.results_filename
    f2=open(mod_tsv,'w')
    ...
    os.system('mv Mod_contig_id.txt Bin_name_mod.txt '+mod_tsv+' '+pwd+'/'+str(binset_folder)+'_mod')
...
def parse_sam_bwa(sam_file, fq, pair, n, batch, mp_run, num_threads=4):
    """
    Single-pass BWA-SAM parser. Caches (count, R1_record, R2_record) per
    (bin_id, read_id) in one scan; rids with exactly 2 hits are emitted as
    paired R1/R2 records, others go to the not-mapped log (preserving the
    original strict-pair semantics including multimapper rejection).
    Buffers are flushed once per output FASTQ file with a thread pool.
    """
    print('Parsing', sam_file)
    f_not_mapped_reads=open('Not_mapped_reads_'+str(batch)+'_'+str(mp_run)+'.txt','a')

    cache={}                # bin_id -> {rid: [count, rec1_or_None, rec2_or_None]}
    m=0
    for line in open(sam_file,'r'):
        ...
        if bin_id_o == '*':
            if m % 1000000 == 0:
                print('Parsed', m,'lines')
            continue
        bin_id=bin_id_o.split('_')[0]
        read_id=flist[0]
        rid=str(n)+'_'+read_id
        b_cache=cache.setdefault(bin_id,{})
        slot=b_cache.get(rid)
        if slot is None:
            fq_seq=flist[9]+'\n+\n'+flist[10]+'\n'
            rec1='@'+rid+' 1\n'+fq_seq
            b_cache[rid]=[1, rec1, None]
        else:
            slot[0]+=1
            if slot[0] == 2:
                fq_seq=flist[9]+'\n+\n'+flist[10]+'\n'
                slot[2]='@'+rid+' 2\n'+fq_seq
            # count > 2 → multimapper, leave; rejected at end

        if m % 1000000 == 0:
            print('Parsed', m,'lines')

    buffers={}              # (bin_id, '1'/'2') -> [record, ...]
    for bin_id, b_cache in cache.items():
        b_fq=fq.setdefault(bin_id,{})
        for rid, slot in b_cache.items():
            if slot[0] == 2:
                b_fq[rid]=0
                buffers.setdefault((bin_id,'1'),[]).append(slot[1])
                buffers.setdefault((bin_id,'2'),[]).append(slot[2])
            else:
...
def parse_sam(sam_file, fq, pair, n, batch, mp_run, num_threads=4):
    """
    Single-pass SAM parser for paired-end alignments.

    For each (bin_id, read_id) the first SAM hit is queued as the R1 record
    and the second hit as the R2 record; both are emitted to per-bin write
    buffers. Buffers are flushed once at the end with one open/close per
    output FASTQ file, parallelised with a thread pool. Updates ``fq`` and
    ``pair`` in place to preserve the original API contract.
    """
    print('Parsing', sam_file)
    f_not_mapped_reads=open('Not_mapped_reads_'+str(batch)+'_'+str(mp_run)+'.txt','a')

    pending={}              # bin_id -> {rid: record_str_for_R1}
    buffers={}              # (bin_id, '1' or '2') -> [record, ...]
    m=0
    for line in open(sam_file,'r'):
        m+=1
        flist=line.split('\t')
        ...
        if rid in b_fq:
            # already produced both R1+R2 for this rid → multimapper, log & drop
            f_not_mapped_reads.write(rid+'\n')
        else:
            fq_seq=flist[9]+'\n+\n'+flist[10]+'\n'
            reads=str(n)+'_'+read_id
            if rid in b_pending:
                rec1=b_pending.pop(rid)
                rec2='@'+reads[:-2]+' 2\n'+fq_seq
                buffers.setdefault((bin_id,'1'),[]).append(rec1)
                buffers.setdefault((bin_id,'2'),[]).append(rec2)
                b_fq[rid]=2
            else:
                b_pending[rid]='@'+reads[:-2]+' 1\n'+fq_seq

        if m % 1000000 == 0:
            print('Parsed', m,'lines')

    # rids with only one hit → mirror legacy ``pair`` state and log
...
def parse_lr_sam(sam_file, long_read, sn):
    ...
    Single-pass implementation: one scan over the SAM (filters by ``bin``
    prefix and seq length >=100), one scan over the long-read FASTQ.
    Per-bin ``_lr.fq`` files stay open as a handle dictionary for the whole
    fastq scan so each record is appended via a single write call instead
    of millions of open/append/close cycles.

    Returns
    -------
    dict
        Mapping ``bin_id -> dict(long_read_id -> '')``.
    """
    print('Parsing', sam_file)
    bin_lr, lr_bin = {}, {}
    m=0
    for line in open(sam_file,'r'):
        ...
        if len(flist) >= 12:
            ...
            if 'bin' in bin_id and len(seqs) >= 100:
                read_id=flist[0]
                bin_lr.setdefault(bin_id,{})[read_id]=''
                lr_bin.setdefault(read_id,{})[bin_id]=''
        if m % 1000000 == 0:
            print('Parsed', m,'lines')

    f=open('Bin_long_read'+str(sn)+'.txt','w')
    for bin_id, lrs in bin_lr.items():
        f.write(str(bin_id)+'\t'+str(lrs)+'\n')
    f.close()

    f=open('Long_read_bin'+str(sn)+'.txt','w')
    for lr, bins in lr_bin.items():
        f.write(str(lr)+'\t'+str(bins)+'\n')
    f.close()

    if not bin_lr:
        print('No bin-mapped long reads in '+str(sam_file))
        return bin_lr

    handles={bin_id: open(str(bin_id)+'_lr.fq','a') for bin_id in bin_lr.keys()}
    ...
    for line in open(long_read,'r'):
        ...
        phase=(n1 - 1) % 4
        if phase == 0:
            seq_id=line.strip().split(' ', 1)[0].lstrip('@')
            target_bins=lr_bin.get(seq_id)
            if target_bins is not None:
                for bin_id in target_bins:
                    handles[bin_id].write(line)
        elif target_bins is not None:
            for bin_id in target_bins:
                handles[bin_id].write(line)
            if phase == 3:
                target_bins=None
...
def mapping_sr(total_fa, datasets_list, fq, pair, mapping_tool,
    ...
    if mapping_tool == 'bw2':
        ...
        for item in datasets_list.keys():
            ...
            parse_sam(str(item)+'.sam', fq, pair, n, 1, mp_run, num_threads=num_threads)
        ...
        for item in datasets_list.keys():
            ...
            parse_sam_bwa(str(item)+'.sam', fq, pair, n, 2, mp_run, num_threads=num_threads)
            os.system('rm '+str(item)+'.sam')
...
def bin_comparison(binset_folder, target_folder, origin_checkm, refined_checkm, pwd, type):
    ...
    for bin_name in refined_checkm.keys():
        ...
        print('Orginal bin:', str(bin_name_o2)+' refined bin:', str(bin_name))
        if float(origin_checkm[bin_name_o2]['Contamination']) >= float(refined_checkm[bin_name]['Contamination']):
            ...
            os.chdir(pwd+'/'+target_folder)
            f.write(bin_name_o2+'\t'+str(refined_checkm[bin_name]['Genome size'])+'\t'+str(refined_checkm[bin_name]['Completeness'])+'\t'+str(refined_checkm[bin_name]['Contamination'])+'\t'+str(refined_checkm[bin_name]['contig_size'])+'\n')
    ...
    for bin_name in origin_checkm.keys():
        if bin_name not in removed_bins.keys():
            ...
            f.write(bin_name+'\t'+str(origin_checkm[bin_name]['Genome size'])+'\t'+str(origin_checkm[bin_name]['Completeness'])+'\t'+str(origin_checkm[bin_name]['Contamination'])+'\t'+str(origin_checkm[bin_name]['contig_size'])+'\n')
    os.chdir(pwd)
...
def polishing_main(binset_folder, datasets_list, assembly_list, long_read, batch, mapping_tool, major_run, ram, num_threads):
    pwd=os.getcwd()
    print('Processing bins form bin folder', str(binset_folder))
    print('Processing assemblies', str(assembly_list))
    print('Processing short-reads datasets', str(datasets_list))
    print('Processing long-read(s)', str(long_read))
    print('Processing batch', str(batch))
    print('Using ', str(mapping_tool))
    print('Major run number set to', str(major_run))
    print('Using ram:', str(ram))
            ...
            if len(datasets_list) != 0:
                ...
                if x == 0:
                    ...
                    if len(contig_seq) >= 1:
                        ...
                        backend = _require_backend()
                        os.system(backend.build_cmd(num_threads, str(binset_folder)+'_gf_lr', str(binset_folder)+'_gf_lr_checkm', ext='fa'))
    ...
    # datasets_list2=copy.deepcopy(datasets_list)
    # if len(datasets_list) != 0 or len(long_read) != 0:
    #     if len(long_read) != 0:
    #         per = len(datasets_list)
    #         for reads in long_read:
    #             per+=1
    #             if 'fq' in reads:
    #                 read1=reads.replace('.fq', '_split_R1.fq')
    #                 read2=reads.replace('.fq', '_split_R2.fq')
    #             elif 'fastq' in reads:
    #                 read1=reads.replace('.fastq', '_split_R1.fq')
    #                 read2=reads.replace('.fastq', '_split_R2.fq')

    #             datasets_list[str(per)]=[]
    #             datasets_list[str(per)].append(str(read1))
    #             datasets_list[str(per)].append(str(read2))
                ...
                if mp_run <= major_run: ### Primary run
                    ...
                    if x == 0:
                        ...
                        if mp_run == 1:
                            ...
                            if batch == 1:
                                try:
                                    A=mod_bin(binset_folder+'_gf_lr', pwd)
                                except:
                                    A=mod_bin(binset_folder, pwd) ### did not have binset_folder+'_gf_lr'
                            else: ### batch == 2 or batch == 3
                                A=mod_bin(binset_folder, pwd) ### 'BestBinset_outlier_refined_filtrated_retrieved_MAGs_polished_re-assembly_OLC'
                            ...
                            try:
                                ...
                                # os.system('mkdir BestBinset_outlier_refined_filtrated_retrieved_polished_sr_bins_seq')
...
if __name__ == '__main__': 
    binset_folder='BestBinset_outlier_refined_filtrated_retrieved'
    # binset_folder='bin7_test'
    # binset_folder='1_Opera_unpolished_cat_contigs_MAGs_polished_1'
    assembly_list=['1_assembly_sample1.fa','2_assembly_sample2.fa']
    # assembly_list=['1_Opera_unpolished_cat_contigs.fasta']
    ...
    datasets_list={'1':['PE_r1_sample1.R1.fq','PE_r2_sample1.R2.fq'], '2':['PE_r1_sample2.R1.fq','PE_r2_sample2.R2.fq']}
    ...
    long_read=['sample1.nanopore.fq','sample2.nanopore.fq']
    major_run=3
    num_threads=30
    ram=250
    mapping_tool='bw2' ### bw2 or bwa
