...
"""
Step S5: Deep-learning-based outlier bin removal.

Unified entry for CheckM2 and CheckM backends. Call ``set_qc_backend(name)``
before invoking ``outlier_remover_main`` to select the QC tool.
"""

from Bio import SeqIO
import os, sys, copy, math, glob, gc
import numpy as np
...
def _require_backend():
    if _BACKEND is None:
        raise RuntimeError(
            'S5_Outlier_remover_DL: QC backend not set. '
...
def basic_information(bin_folder, depth_list, kmer_list, pwd, num_threads):
    ...
    # os.system('perl '+_script_path('calc.kmerfreq.pl')+' -i BestBinset.fasta -o BestBinset.kmer.txt')
...
def outlier_predictor(depth_TNF_matrix, contigs_depth, bin_contigs,
    ...
    rc = os.system(sys.executable + ' -m basalt.ml.ensemble')
    if rc != 0 or not os.path.exists('Predicted_potential_outlier.txt'):
        raise RuntimeError(
            'basalt.ml.ensemble failed (exit {}); '
            'Predicted_potential_outlier.txt was not produced.'.format(rc))
    ...
    for line in open('Re-mapped_depth.txt','r'):
        n+=1
        if n == 1:
            num=sum(1 for f in str(line).rstrip("\r\n").split("\t") if f.endswith("-var"))
            # nx=int(int(num)/2)
        else:
            contig_id=str(line).strip().split('\t')[0]
...
def checkm_eval(bin_contigs, bin_folder, confirmed_outlier, pwd, num_threads):
    ...
    os.system(backend.build_cmd(num_threads, str(outlier_binset), str(outlier_binset)+'_checkm', ext='fa'))
    print('Parsing '+outlier_binset+' '+backend.name+' output')
    ...
    for binID in refined_checkm.keys():
        f.write(str(binID)+'\t'+str(refined_checkm[binID]['Genome size'])+'\t'+str(refined_checkm[binID]['Completeness'])+'\t'+str(refined_checkm[binID]['Contamination'])+'\t'+str(refined_checkm[binID]['contig_size'])+'\n')
    f.close()
    os.chdir(pwd)
...
def outlier_remover_main(bin_folder, depth_list, datasets, assemblies_list,
                         pwd, num_threads, lr=None, hifi_list=None):
    """
    Entry point for S5: DL-based outlier removal.
    ...
    lr : list of str, optional
        Long-read datasets (ONT/PB). Used only by CheckM2 outlier prediction.
    hifi_list : list of str, optional
        HiFi read datasets. Used only by CheckM2 outlier prediction.
    """
    if lr is None:
        lr = []
    if hifi_list is None:
        hifi_list = []
    kmer_list=[]
    pool=Pool(processes=num_threads)
    ...
    del contigs_depth, ccc, bin_contigs, contig_status, bin_avg_coverage, confirmed_outlier
    gc.collect()
