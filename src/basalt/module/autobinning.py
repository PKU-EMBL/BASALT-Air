...
from basalt.steps.s1_autobinners import *
from basalt.steps.s1e_extra_binners import *
from basalt.steps.s2_bins_abundance_pe_connections import *
from basalt.steps.s2_bins_abundance_pe_connections import set_qc_backend as _set_s2_backend
from basalt.steps.s3_bins_comparator_within_group import *
...
def BASALT_main_autobinning(assembly_list, datasets, num_threads, lr_list, hifi_list,
    ...
    """
    Run the BASALT autobinning module. The QC backend (CheckM2 or
    ...
    _set_s2_backend(QC_software)
    _set_s3_backend(QC_software)
    ...
    if functional_module == 'autobinning' or functional_module == 'all':
        if last_step == 0:
            ...
            # A=autobinner_main(assembly_list, datasets, lr_list, num_threads, ram, autobining_parameters, pwd)
            # A=autobinner_main(assembly_list, datasets, num_threads, ram, autobinning_parameters, pwd)
        ...
        if last_step < 3:
            if len(assembly_list) != 1:
                ...
                # multiple_assembly_comparator_main(assembly_mo_list, bestbinset_list, coverage_matrix_list, num_threads)
                datasets_fq={}
                for item in datasets.keys():
                    datasets_fq[item]=[]
                    datasets_fq[item].append('PE_r1_'+str(datasets[item][0]))
                    datasets_fq[item].append('PE_r2_'+str(datasets[item][1]))
                multiple_assembly_comparator_main(assembly_mo_list, bestbinset_list, coverage_matrix_list, datasets_fq, 'initial_drep', num_threads)
...
if __name__ == '__main__':
    ...
    sensitive='sensitive'
    functional_module='autobinning'
    continue_mode='last'
    max_ctn, min_cpn=20, 35
    QC_software='checkm2'
    output_folder='Final_binset'
    BASALT_main_autobinning(assembly_list, datasets, num_threads, lr_list, hifi_list, hic_list, eb_list, ram, continue_mode, functional_module, sensitive, refinement_paramter, max_ctn, min_cpn, pwd, QC_software, output_folder)
